...
function generateNetworthData(userId) {
  const records = [];
  ...
  // Growth rates per month (some volatility for realistic data)
  ...
  for (let month = 0; month < 12; month++) {
    ...
    for (let record = 0; record < 5; record++) {
      const dayOfMonth = Math.floor((record + 1) * (28 / 5)) + 1; // Spread across month (1-28)
...
async function seedNetworthHistory() {
  try {
    ...
    console.log(`📊 Generating data for user: ${firstUser.email || firstUser.number} (ID: ${firstUser.id})`);
    
    // Check if net worth history already exists for this user in 2025
    ...
    if (existingRecords > 0) {
      console.log(`⚠️  Found ${existingRecords} existing 2025 records. Clearing them first...`);
    ...
    // Insert all records in batches to avoid potential issues
    console.log('💾 Inserting records into database...');
    
    let insertedCount = 0;
    const batchSize = 10;
    
    for (let i = 0; i < networthData.length; i += batchSize) {
      const batch = networthData.slice(i, i + batchSize);
      const result = await prisma.networthHistory.createMany({
        data: batch,
        skipDuplicates: true
      });
      insertedCount += result.count;
      console.log(`   Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(networthData.length / batchSize)} (${result.count} records)`);
    }
    
    console.log(`✅ Successfully inserted ${insertedCount} net worth history records!`);
    ...
    console.log(`\n📊 Statistics:`);
    console.log(`   • Starting Net Worth: $${networthData[0].netWorth.toLocaleString()}`);
    console.log(`   • Ending Net Worth: $${networthData[networthData.length - 1].netWorth.toLocaleString()}`);
    ...
    console.log(`   • Records per Month: 5`);
...
seedNetworthHistory()
  .then(() => {
    console.log('\n🎉 Net worth history seeding completed successfully!');
    console.log('💡 You can now view the chart with historical data in your app.');
    process.exit(0);
  })
  .catch((error) => {
