...
interface DeleteSelectedScheduledPaymentsModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function DeleteSelectedScheduledPaymentsModal({
  isOpen,
  onClose,
  selectedCount,
  onConfirm,
}: DeleteSelectedScheduledPaymentsModalProps) {
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="delete-selected-scheduled-payments-title"
        onClick={(e) => e.stopPropagation()}
      >
        <h2
          id="delete-selected-scheduled-payments-title"
          className="text-lg font-semibold text-gray-900"
        >
          Delete selected scheduled payments?
        </h2>
        <p className="mt-2 text-sm text-gray-600">
          This will permanently remove {selectedCount} selected scheduled payment
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="mt-4 space-y-3">
          <div>
            <label
              htmlFor="delete-selected-scheduled-screen-lock"
              className="mb-1 block text-sm font-medium text-gray-700"
            >
              Screen lock password
            </label>
            <input
              id="delete-selected-scheduled-screen-lock"
              type="password"
