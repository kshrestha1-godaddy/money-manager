...
import { 
    parseCategoriesCSV, 
    type ParsedCategoryData 
} from "../../../utils/csvImportCategories";
import { 
    ImportResult, 
    ImportExpenseRow, 
    BulkOperationResult 
...
/**
 * Combined import: Import categories first, then expenses
 */
export async function bulkImportExpensesWithCategories(
    expenseFile: File, 
    categoriesFile?: File, 
    defaultAccountId?: string
): Promise<{
    expenseImport: ImportResult;
    categoryImport?: {
        success: number;
        errors: Array<{ row: number; error: string }>;
        importedCount: number;
    ...
    // Import expenses
    const expenseText = await expenseFile.text();
    const expenseImportResult = await bulkImportExpenses(expenseText, defaultAccountId ? parseInt(defaultAccountId) : undefined);

    return {
        expenseImport: expenseImportResult,
