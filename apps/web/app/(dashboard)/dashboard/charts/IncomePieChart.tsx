...
export const IncomePieChart = React.memo<IncomePieChartProps>(({ currency = "USD", title, heightClass }) => {
    ...
    const { formatTimePeriod, filteredIncomes } = useChartData();
    ...
    const renderCustomizedLabel = useCallback(
        (entry: {
            ...
            const labelColor = chartDataEntry?.solidColor ?? chartDataEntry?.color ?? entry.fill ?? "#374151";
    ...
    const ChartContent = () => (
        <div className="px-1 py-4 sm:px-2 sm:py-5">
            <div className="mb-3 sm:mb-4">
                <p className="text-xs text-gray-600 sm:text-sm">Total Income</p>
                <p className="text-lg font-semibold text-green-600 sm:text-xl">{formatCurrency(total, currency)}</p>
            </div>
