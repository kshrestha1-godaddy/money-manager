            ...
            <div className="space-y-2">
                {!previewUrl ? (
                    ...
                    <button
                        ...
                        disabled={disabled || isUploading}
                        className={`
                            w-full p-6 border-2 border-dashed rounded-lg text-center transition-all
                            ${disabled || isUploading 
                                ? 'border-gray-200 text-gray-400 cursor-not-allowed' 
                                : 'border-gray-300 text-gray-600 hover:border-[#705ba0] hover:text-[#705ba0] cursor-pointer'
                            }
                        `}
                    >
                        <div className="space-y-3">
                            <div className="mx-auto w-12 h-12 text-gray-400">
                                {isUploading ? (
                                    <div className="flex items-center justify-center w-full h-full">
                                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#705ba0]"></div>
                                    </div>
                                ) : (
                            ...
                            <div className="text-sm">
                                {isUploading ? (
                                    ...
                                    <>
                                        <span className="font-medium text-[#705ba0]">Click to upload an image</span>
                        ...
                        <div className="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-40 transition-all duration-200 flex items-center justify-center opacity-0 hover:opacity-100">
                            <div className="space-x-2">
                                <button
                                    ...
                                    onClick={triggerFileSelect}
                                    disabled={disabled || isUploading}
                                    className="px-4 py-2 bg-[#705ba0] text-white text-sm rounded-lg hover:bg-[#5d4a87] disabled:opacity-50 transition-colors"
                                >
                                    Replace
                                </button>
                                <button
                        ...
                        {isUploading && (
                            <div className="absolute inset-0 bg-white bg-opacity-80 flex items-center justify-center">
                                <div className="text-center">
                                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#705ba0] mx-auto mb-2"></div>
