...
export function SankeyChart({ data, currency = "USD", title, startDate, endDate, type }: SankeyChartProps) {
    const { isExpanded, toggleExpanded } = useChartExpansion();
    const chartRef = useRef<HTMLDivElement>(null);
    const chartInstance = useRef<any>(null);
    const chartId = useRef(`${type}-sankey-${Math.random().toString(36).substring(7)}`);
    ...
    // Group data by category and sum amounts, preserving category colors
    const categoryMap = new Map<string, { amount: number; color: string }>();
    
    data.forEach(item => {
        const categoryName = item.category?.name || 'Unknown Category';
        const color = item.category?.color || '#6366f1'; // Default color if not available
        const current = categoryMap.get(categoryName) || { amount: 0, color };
        categoryMap.set(categoryName, { 
            amount: current.amount + item.amount, 
            color: color // Keep the color from the category
        });
    });

    // Convert to Sankey data format
    const sankeyData: SankeyData[] = [];
    const total = Array.from(categoryMap.values()).reduce((sum, value) => sum + value.amount, 0);
    
    // Create flow from each category to total (different for income vs expense)
    const totalLabel = type === 'income' ? "Total Income" : "Total Expenses";
    Array.from(categoryMap.entries()).forEach(([categoryName, { amount, color }]) => {
        sankeyData.push({
            from: categoryName,
            to: totalLabel,
            size: amount,
    ...
    // Sort all categories by amount (descending) first
    const sortedSankeyData = sankeyData.sort((a, b) => b.size - a.size);
    ...
    // Create "Other Sources/Categories" if there are small categories
    const finalSankeyData = [...significantCategories];
    if (smallCategories.length > 0) {
        const othersTotal = smallCategories.reduce((sum, item) => sum + item.size, 0);
        const othersLabel = type === 'income' ? "Other Sources" : "Other Categories";
    ...
    useEffect(() => {
        ...
        const drawChart = () => {
            ...
            try {
                ...
                // Ensure final data is sorted by size (descending) for consistent flow ordering
                const sortedFinalData = finalSankeyData.sort((a, b) => b.size - a.size);
                
                // Add rows from our data with custom tooltips and percentage labels
                const rows = sortedFinalData.map((item, index) => {
                ...
                // Extract colors from the sorted data (use actual category colors)
                const flowColors = sortedFinalData.map(item => item.color || '#6366f1');
                ...
                const options = {
                    ...
                    sankey: {
                        ...
                        link: {
                            colorMode: 'source'  // Each link takes color from its source node
                        }
                    },
                    tooltip: {
                        isHtml: true
    ...
    }, [finalSankeyData, isExpanded, type]);

    const defaultTitle = type === 'income' ? 'Income Distribution' : 'Expense Distribution';
    const chartTitle = title || `${defaultTitle} ${timePeriodText}`;
    const tooltipText = type === 'income' 
        ? 'Flow visualization of your income sources contributing to total income'
        : 'Flow visualization of your expense categories contributing to total expenses';
                ...
                if (ctx) {
                    ...
                    canvas.toBlob((blob) => {
                        if (blob) {
                            ...
                            link.download = `${type}-sankey-chart.png`;
                            link.href = url;
                            link.click();
    ...
    const csvDataForControls = [
        [type === 'income' ? 'Income Source' : 'Expense Category', 'Amount', 'Percentage'],
            ...
            {finalSankeyData.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-gray-500">
                    ...
                    <p className="text-sm text-center max-w-sm">
                        Add some {type} entries to see the flow visualization.
                        Your {type === 'income' ? 'income sources' : 'expense categories'} will appear here.
                    </p>
                </div>
            ) : (
                <div className={isExpanded ? "h-full" : ""}>
